...
| How is agentic behaviour used? *Using an agent where a single call would do is not automatically better.* | LangGraph state machine. Agents only where the task is genuinely multi-step: extraction, conflict adjudication, repair. Emission and validation are plain code. | ADR-004 |
